...
**記録とフラグは実装済み（2026-09-09）。残っているのは「打刻を弾く」ところだけ。**

実装済み:
- 打刻時にブラウザGeolocation APIで位置を取得し、`AttendanceEvent` に保存（店舗設定でONにした店だけ）
- 店舗座標と判定距離を設定すると、離れた打刻に異常フラグが付く（`LOCATION_OUT_OF_RANGE`）
- リモート出勤のスタッフは `StaffStore.skipLocationCheck` で判定から外せる

未実装（判断待ち）:
- **店舗から離れていると打刻できないようにする**。実分布を1ヶ月見てから決める。
  ビル内では測位がWi-Fi/基地局にフォールバックして誤差が数百m〜数kmになるため、
  距離だけで弾くと本人が打刻できず、その分が管理者の手入力に化けて精度がむしろ落ちる
- ブロックを入れるなら、店舗ごとに 記録のみ / 警告 / ブロック を選べる形にし、
  位置が取れないときの代替経路（店長発行の一時解除コード等）を必ず残すこと
- 店舗のグローバルIPによる判定。ビル内ではGPSより精度も安定性も上になる可能性がある
